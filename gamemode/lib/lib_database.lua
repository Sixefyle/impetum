...
local base_table = GST_DB
local prefix = "GST_DB."

base_table.DatabaseMode = "mysqloo"
...
base_table.DatabaseConfig = {
	host = "panel.gst-games.net",
	user = "u113_bTktR2EaBn",
	password = "EO.OiP7O^8GBIhg!Yk45zfJC",
	database = "s113_GST_SNK",
	port = 3306,
}
...
function base_table:Query(query, callback)
	...
	if (dm == "mysqloo") then
		...
		q.onError = function(me, err)
			callback(query, false, err)
