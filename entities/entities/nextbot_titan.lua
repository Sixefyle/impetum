...
function ENT:Initialize()
	...
	local realSize = table.Random(self.Height) / self.HumanBaseHeight
...
function ENT:FindEnemy()
    ...
    for k, v in ipairs(_ents) do
        if v:IsPlayer() and not v:IsFlagSet(FL_NOTARGET) then
...
function ENT:ChaseEnemy(options)
    ...
    while path:IsValid() and self:HaveEnemy() do
        ...
        self:ManipulateBoneAngles(self:LookupBone("mixamorig:Spine"),
            Angle(0, 0, (self:GetEnemy():GetPos() - self:EyePos()):Angle()[1] + (self:GetModelScale() * 3) ))
...
function ENT:AttackEnemy()
    ...
    if (distanceFromFeets <= 50 * self:GetModelScale()) then
        ...
        self:ApplyDamage("mixamorig:RightFoot", animTime)
        self:PlaySequenceAndWait("kick", 1.8)
    else
        _, animTime = self:LookupSequence("punch1")
        self:ApplyDamage("mixamorig:RightHand", animTime)
...
function ENT:ApplyDamage(bone, time)
    local damagedPlayers = {}
    local buildDestroyed = false
    timer.Create("checkNearbyPlayer" .. self:EntIndex(), .1, time * 10, function()
        local bonePos = self:GetBonePosition(self:LookupBone(bone))

        for _, ply in pairs(ents.FindInSphere(bonePos, 80)) do
            if IsValid(ply) and ply:IsPlayer() and not table.HasValue(damagedPlayers, ply) and ply ~= self and ply:Alive() then
                ply:TakeDamage(100, self, self)
...
hook.Add( "PostDrawTranslucentRenderables", "MySuper3DRenderingHook", function()
    -- for _, ent in pairs(ents.FindByClass("nextbot_titan")) do
    --     if (IsValid(ent)) then
    --         render.DrawLine(ent:EyePos(), ent:GetPos() + ent:EyeAngles():Forward() * 200 + Vector(0,0,300), Color(255,0,0) )
    --     end
    -- end
end )