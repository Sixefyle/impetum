...
hook.Add("PlayerFootstep", "GiantessFootstep", function(ply, pos, foot, sound, volume, rf)
    if ply:GetStepSize() == 170 then
        if SERVER then
            ply:EmitSound(StepSound, 45)
...
hook.Add("PlayerStepSoundTime", "GiantessStepSoundTime", function(ply, iType, bWalking)
    if ply:GetStepSize() == 170 then
        local fStepTime = 350
...
function SWEP:Deploy()
    ...
    if IsValid(ply) then
        if SERVER then
            ...
            if size == 3 then
                jump = true
                --ply:SetJumpPower( 1400 )
                --ply:SetRunSpeed( 800 )
            else
                ply:SetJumpPower(850)
                ply:SetRunSpeed(650)
            end
...
function SWEP:PrimaryAttack()
    ...
    self.Owner:EmitSound(SwingSound, 40)
    ...
    timer.Simple(0.01, function()
        if SERVER then
            if not IsValid(wep) or not ply:Alive() then return end
            local tr = {}
            ...
            if trace.Hit then
                ...
                elseif ent:IsNPC() then
                    ...
                    ent:TakeDamage(math.random(20, 40), ply, ply)
                    ply:EmitSound("physics/flesh/flesh_squishy_impact_hard3.wav", 30, 100)
                -- else
                --     ent:SetVelocity(ply:GetForward() * 2000)
